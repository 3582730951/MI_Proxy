...
func main() {
	report := map[string]any{
		...
		"items": []item{
			...
			{Category: "deploy", Name: "Dockerfile, docker-compose without default DB password, localhost-bound service ports, one-command bootstrapper, zero-interaction VPS install script with runtime passwd.txt password file, safe auto-update script with configurable password file, health check, and rollback, PostgreSQL schema and indexes", Status: "container-test-passed", Evidence: "go test ./tests; Dockerfile; docker-compose.yml; migrations/001_core.sql; scripts/bootstrap.sh; scripts/install.sh; scripts/update.sh"},
