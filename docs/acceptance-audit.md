...
- UI navigation, authenticated API session controls, core metrics visualization, capacity/autoscaling/cost signals, live node/WARP traffic map, route decision audit drill-down, and dark operations dashboard styling are bundled at `/` without adding unauthenticated admin APIs. Frontend tests block demo operational data and require runtime API-backed rendering: `apps/web/index.html`, `apps/web/app.js`, `apps/web/styles.css`, `apps/web/ui_test.go`
