...
- Subscription generation by user, device, region, protocol, outbound policy, embedded CN/private direct rules, Google Scholar WARP exclusion, split DNS strategy, Clash/Mihomo importable proxy fields and rule-providers, token kinds, IP allowlist, one-time revocation, ETag / Last-Modified, gzip, and brotli support: `services/controlplane/controlplane.go`, `services/controlplane/api_test.go`, `services/controlplane/controlplane_test.go`
...
- UI navigation, Chinese account/password login, authenticated API session controls, first-run readiness checks, API failure redaction, safe VPS runtime information, default subscription metadata, session-only subscription copy actions, core metrics visualization, capacity/autoscaling/cost signals, live node/WARP traffic map, route decision audit drill-down, and dark operations dashboard styling are bundled at `/` without adding unauthenticated admin APIs. Frontend tests block demo operational data, obsolete Header/Token login UI, direct subscription token rendering, and require runtime API-backed rendering: `apps/web/index.html`, `apps/web/app.js`, `apps/web/styles.css`, `apps/web/ui_test.go`
- Docker deployment, non-root runtime image, Compose deployment without default database or panel admin passwords, localhost-bound service ports, one-command bootstrapper, zero-interaction VPS installer with generated password file defaulting to runtime `passwd.txt`, generated default subscription token kept out of the frontend, and auto-update script with configurable password file, generated admin/default subscription credentials for older installs, fast-forward pull, health check, and rollback: `Dockerfile`, `docker-compose.yml`, `scripts/bootstrap.sh`, `scripts/install.sh`, `scripts/update.sh`
